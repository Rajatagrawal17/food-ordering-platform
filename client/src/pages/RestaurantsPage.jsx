      ...
      {!loading && !error && (
        <div className="checkout-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', width: '100%', gap: '24px' }}>
          {restaurants.map((restaurant) => (
            <article className="card" key={restaurant._id} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <div className="card__actions" style={{ marginTop: '12px', padding: '0 16px 16px 16px' }}>
                <Link to={`/restaurants/${restaurant._id}`} className="button button--primary" style={{ width: '100%', textAlign: 'center', display: 'block', padding: '10px 0', borderRadius: '12px' }}>
                  View Menu
                </Link>
              </div>
            </article>
